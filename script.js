...
function moveNoButton(fromClick) {
    ...
    const margin = window.matchMedia("(max-width:520px)").matches ? 18 : 14;
    ...
    const minDist = window.matchMedia("(max-width:520px)").matches ? 130 : 110;
    const maxDist = window.matchMedia("(max-width:520px)").matches ? 220 : 200;
...
noBtn.addEventListener("pointerenter", () => moveNoButton(false));
...
window.addEventListener("resize", () => {
    if (document.getElementById("noBtn")) moveNoButton(false);
});

setTimeout(() => { if (document.getElementById("noBtn")) moveNoButton(false); }, 200);